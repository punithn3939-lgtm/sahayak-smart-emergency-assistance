...
-- Adds separate hospital onboarding, capacity, availability, recommendations and estimates.
...
create policy "hospital applications own read"
...
create policy "hospital applications own insert"
on public.hospital_applications for insert
to authenticated with check (auth.uid() = user_id);

-- Public authenticated discovery only exposes operational summary, not patient data.
...
-- Hospital staff can update only their assigned hospital operational fields.
drop policy if exists "hospital staff update own hospital" on public.hospitals;
create policy "hospital staff update own hospital"
...
-- When a hospital signup is created, keep it pending instead of granting hospital privileges.
...
-- Safe approval helper: only service-role/SQL admin should call this in a real deployment.
...
-- Replace routing with an availability/capacity/rating aware selector.
create or replace function public.route_sos_to_hospital()
returns trigger language plpgsql security definer set search_path = public as $$
declare
  selected_hospital uuid;
  ...
  condition_text text;
begin
  select h.id into selected_hospital
  from public.hospitals h
  where h.available = true
    and h.emergency_available = true
    and exists (
  ...
  order by (
    case when new.latitude is null or new.longitude is null then 0
         else 1 / (1 + sqrt(power((h.latitude-new.latitude)*111,2) + power((h.longitude-new.longitude)*111*cos(radians(coalesce(new.latitude,h.latitude))),2))) end
    + case when h.icu_beds_available > 0 then 0.25 else 0 end
    + case when h.ambulance_available then 0.15 else 0 end
    + least(h.rating / 10.0, 0.5)
  ) desc
  limit 1;
...
-- Seed useful demo capacity/estimate values if the demo hospital exists.
update public.hospitals
set emergency_available=true,
    ambulance_available=true,
    icu_beds_available=4,
    general_beds_available=20,
    specialties=array['Emergency Medicine','Trauma','Cardiology','General Medicine'],
    estimated_charge_low=1500,
    estimated_charge_high=15000,
    rating=4.4,
    last_capacity_update=now()
where name='Sahayak Demo Hospital';

-- Realtime is already enabled for emergency_cases by v1; also stream hospital capacity changes.
do $$ begin
  if not exists (select 1 from pg_publication_tables where pubname='supabase_realtime' and schemaname='public' and tablename='hospitals') then
